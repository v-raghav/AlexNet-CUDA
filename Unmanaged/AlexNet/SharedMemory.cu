...
void DeviceAllocate(float **I, float **W,float **O, ConvParam **Param_d,CudaParamType **CudaParam_d, const ConvParam *Param);
...
int main() {
    ...
    DeviceAllocate(&I_d,&kernel_d,&O_d, &Param_d,&CudaParam_d,&Param_h);
    HostToDevice(I_d, kernel_d, O_d, Param_d,CudaParam_d, I_h, kernel_h, O_h, &Param_h,&CudaParam_h);
   
    dim3 blocksize(TILE_WIDTH,TILE_WIDTH,1);
    dim3 gridsize(ceil((float)(Param_h.outHeight)/TILE_WIDTH)*ceil(((float)Param_h.outWidth)/TILE_WIDTH),Param_h.outChannels,Param_h.batchSize);




     //Convolution
    convWithGPUShMem_kernel<<<gridsize,blocksize,CudaParam_h.shMemSizeTotal*sizeof(float) >>>(I_d,kernel_d,O_d,Param_d,CudaParam_d);
...
void DeviceAllocate(float **I, float **W,float **O, ConvParam **Param_d,CudaParamType **CudaParam_d, const ConvParam *Param_h) {
